...
class Partner(models.Model):
	...
	state_partner = fields.Selection(servicio_busqueda.STATE, string='Estado', default="ACTIVO")
	...
	def _doc_number_change(self):
         for record in self:
            _logger.info("🔎 [ONCHANGE] Ejecutando _doc_number_change para partner %s", record.id)
            ...
            if record.busqueda_automatica is False:
                _logger.info("⏩ Busqueda automática desactivada, solo sincronizando ZIP si falta...")
                if record.l10n_pe_district and not record.zip:
                    ...
                    _logger.info("✅ ZIP actualizado manualmente: %s", record.zip)
                return
            ...
            _logger.info("🔑 Token: %s, Tipo busqueda: %s", token, tipo_busqueda_ruc_dni)
                ...
                if vat_type == "1":
                    ...
                    try:
                        if tipo_busqueda_ruc_dni == "apiperu":
                            ...
                            response = servicio_busqueda.get_dni_apinet(token, vat.strip())
                            _logger.info("🌐 Respuesta API.NET DNI: %s", response)
                    ...
                    if response:
                        ...
                        _logger.info("🎉 Nombre asignado desde servicio: %s", response)
                    ...
                    if len(district) == 1:
                        ...
                        province = record.env["res.city"].search([("name", "ilike", "Lima")])
                        if len(province) == 1:
                ...
                # === RUC ===
                    ...
                    if vals:
                        ...
                        if vals.get("buen_contribuyente", False):
                            record.buen_contribuyente = vals.get("buen_contribuyente")
                        ...
                        if vals.get("ubigeo"):
                            ...
                            _logger.info("📍 Distrito por ubigeo %s: %s", ubigeo, district)
                            ...
                            _logger.info("📍 Distrito por nombre %s: %s", distrito, district)
                            ...
                            if len(district) < 1:
                                ...
                                district = ditrict_obj.search([
                                    ("name_simple", "=ilike", distrito),
                                    ("city_id.name_simple", "=ilike", vals.get("provincia"))
                                ])
                            if len(district) > 1:
                                raise Warning("Más de un distrito encontrado para " + distrito)
                            elif len(district) < 1:
                                raise Warning("No se pudo ubicar distrito " + distrito)
                        ...
                        if district:
                            ...
                            _logger.info("✅ Ubicación asignada desde distrito: %s", district)

            # Sincronizar ZIP si falta
            if record.l10n_pe_district and not record.zip:
                record.write({"zip": record.l10n_pe_district.code})
                record.zip = record.l10n_pe_district.code
                _logger.info("📮 ZIP actualizado desde distrito: %s", record.zip)
